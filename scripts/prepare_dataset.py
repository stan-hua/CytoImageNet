from preprocessor import create_image, get_file_references
...
def to_grayscale(x):
    ...
    if channels_meaningful:
        ...
        # Average along channels. Then normalize to 0-255
        img = img.mean(axis=-1)
        img = img * 255
    else:
...
def create_crops(img, num_crops: int):
    ...
    x_mins, x_maxs, y_mins, y_maxs = get_slicers(img.size[0], img.size[1],
                                                 num_crops, 70, 70)
    ...
    for i in range(len(x_mins)):
        ...
        if sum(img.convert("L").getextrema()) <= 480 and sum(img.convert("L").getextrema()) >= 30:
            img_crops.append(img.crop((x_mins[i], y_mins[i], x_maxs[i], y_maxs[i])))
            used_xmins.append((x_mins[i]))
            used_xmaxs.append((x_maxs[i]))
            used_ymins.append((y_mins[i]))
            used_ymaxs.append((y_maxs[i]))

    return img_crops, (used_xmins, used_xmaxs, used_ymins, used_ymaxs)
...
def supplement_label(label: str):
    ...
    df_label_up = df_label.assign(crop=False, x_min=None, x_max=None, y_min=None, y_max=None)
    ...
    for i in range(len(df_label)):
        ...
        # Load image
        img = Image.open(df_label.iloc[i].path + "/" + df_label.iloc[i].filename)
        ...
        if img.size[0] >= 140 and img.size[1] >= 140:
            ...
            df_crops = df_crops.assign(crop=True, x_max=crop_info[1], y_min=crop_info[2], y_max=crop_info[3], filename=new_filenames)

            # Update metadata dataframe w/ new crops. Replace original image
...
if __name__ == '__main__' and "D:\\" not in os.getcwd():
    ...
    a_pool.map(main, files[len(files)//3:2*len(files)//3])


    # for file in files[2*len(files)//3::-1]:


        # # Check if there are RGB images
        # # Sample 2 rows from each dataset present
        # name_samples = df[exists_series].groupby(by=["name"]).sample(n=2)
    ...
    print("Number of Labels w/ >= 287 Images: ", num_valid)
